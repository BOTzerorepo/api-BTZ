...
<head>
  ...
  <title>Carga en zona de aduana</title>
  <meta name="x-preheader" content="Carga en zona de aduana — Detalles para {{ $datos['booking'] }}"/>
  ...
  <style>
    ...
    .table{width:100%}
    .tdk{width:42%;vertical-align:top;padding:10px;background:#FFFFFF}
    .tdv{width:58%;vertical-align:top;padding:10px;background:#FFFFFF}
    .row-alt .tdk,.row-alt .tdv{background:#FAFAFA}
    .badge{display:inline-block;padding:6px 10px;border-radius:6px;background:#E6F4EA;color:#1D944E;font-weight:700}
...
<body>
  ...
  <span style="display:none!important;visibility:hidden;opacity:0;color:transparent;height:0;width:0;overflow:hidden;">
    Carga en zona de aduana — Detalles para {{ $datos['booking'] }}
  </span>
          ...
          <tr>
            <tr>
              <td class="center p-16" style="background:#e0e0e0;">
                <div class="tag">CARGA EN ADUANA</div>
                <h1 class="h1" style="margin-top:6px;">La unidad se encuentra fuera de @if( $datos['custom_place_impo'] != null )
  
                  {{$datos['custom_place_impo']}}
        ...
        <table role="presentation" class="container" width="100%">
          <tr>
            <td class="p-24">
              ...
              <!-- Encabezado de CNTR + Booking en formato tabla -->
              <table class="table" role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:12px;">
                <tr>
                  <td class="tdk">CNTR</td>
                  <td class="tdv">
                    @if( $datos['confirmacion'] != 0)
                      <span class="badge">{{ $datos['cntr'] }}</span>
                    @else
                      <span class="badge" style="background:#FFF3CD;color:#8a6d3b;">SIN CONFIRMAR</span>
                    @endif
                  </td>
                </tr>
                <tr class="row-alt">
              ...
              <!-- Descripción -->
              <p class="lead" style="text-align:center;margin-top:16px;font-size:16px;">
                {{ $datos['description']}}
              </p>

              <hr class="hr"/>
              <p class="muted" style="margin:0;text-align:center;">
                Status informado por <strong>{{ $datos['user']}}</strong> — Creado el {{ $datos['date']}}
              </p>
            </td>
          </tr>
        </table>

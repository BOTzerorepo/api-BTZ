...
<head>
  ...
  <title>Nueva Impo Marítima</title>
  <meta name="x-preheader" content="Nueva Impo Marítima para {{ $datos['trader'] ?? 'cliente' }} — detalles y estado."/>
...
<body>
  ...
  <span style="display:none !important;visibility:hidden;opacity:0;color:transparent;height:0;width:0;overflow:hidden;">
    {{ 'Nueva Impo Marítima para ' . ($datos['trader'] ?? 'cliente') . ' — detalles y estado.' }}
  </span>

  <table class="wrapper" role="presentation" width="100%" cellpadding="0" cellspacing="0">
    <tr><td style="height:24px;"></td></tr>

    <!-- Header / Logo unificado -->
    <tr>
      <td>
        <table role="presentation" class="container" width="100%">
          <tr>
            <td class="center p-24">
              <!-- Si preferís el logo local, podés reemplazar por {{ asset('image/CorreoAutomaticoTrans.png') }} -->
        ...
        <table role="presentation" class="container" width="100%">
          <tr>
             <td class="center p-16" style="background:#e0e0e0;">
              <div class="tag">Nueva Impo Marítima</div>
          ...
          <tr>
            <td class="p-24">
              ...
              <table class="table" role="presentation" cellpadding="0" cellspacing="0">
                ...
                <tr><td class="tdk">BL / HBL</td><td class="tdv">{{ $datos['bl_hbl'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Trader</td><td class="tdv">{{ $datos['trader'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Importador</td><td class="tdv">{{ $datos['importador'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Cantidad y Tipo de CNTR</td><td class="tdv">{{ $datos['cantidad'] ?? '-' }} x {{ $datos['cntr_type'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Mercadería</td><td class="tdv">{{ $datos['commodity'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Senasa</td><td class="tdv">{{ $datos['senasa'] ?? '-' }} — {{ $datos['senasa_string'] ?? '-' }}</td></tr>
                <tr>
                ...
                <tr> 
                  ...
                  <td class="tdv">
                    ...
                    @endif
                <tr class="row-alt"><td class="tdk">Lugar de Carga</td><td class="tdv">{{ $datos['loadPlace'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Día de Carga</td><td class="tdv">{{ $datos['loadDate'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Lugar de Aduana Impo</td><td class="tdv">{{ $datos['customPlaceImpo'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Fecha Descarga</td><td class="tdv">{{ $datos['cutOffFisico'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Lugar de Entrega</td><td class="tdv">{{ $datos['loadPort'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Despachante Impo</td><td class="tdv">{{ $datos['customAgentImpo'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Armador</td><td class="tdv">{{ $datos['armador'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Devolución de vacío</td><td class="tdv">{{ $datos['finalPoint'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Observaciones</td><td class="tdv">{{ $datos['obeservaciones'] ?? '-' }}</td></tr>
              </table>

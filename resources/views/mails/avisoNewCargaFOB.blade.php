...
<head>
  ...
  <title>Nueva Puesta FOB</title>
  <meta name="x-preheader" content="Nueva Puesta FOB para {{ $datos['trader'] ?? 'cliente' }} — detalles y estado."/>
...
<body>
  <!-- Preheader oculto para preview -->
  <span style="display:none !important;visibility:hidden;opacity:0;color:transparent;height:0;width:0;overflow:hidden;">
    {{ 'Nueva Puesta FOB para ' . ($datos['trader'] ?? 'cliente') . ' — detalles y estado.' }}
  </span>
        ...
        <table role="presentation" class="container" width="100%">
          <tr>
             <td class="center p-16" style="background:#e0e0e0;">
              <div class="tag">Nueva Puesta FOB</div>
          ...
          <tr>
            <td class="p-24">
              ...
              <table class="table" role="presentation" cellpadding="0" cellspacing="0">
                ...
                <tr class="row-alt"><td class="tdk">Shipper</td><td class="tdv">{{ $datos['shipper'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Cantidad y Tipo de CNTR</td><td class="tdv">{{ $datos['cantidad'] ?? '-' }} x {{ $datos['cntr_type'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Depósito de Retiro</td><td class="tdv">{{ $datos['depositoRetiro'] ?? '-' }}</td></tr>
                ...
                <tr class="row-alt"><td class="tdk">Lugar de Aduana Expo</td><td class="tdv">{{ $datos['customPlace'] ?? '-' }}</td></tr>
                <tr><td class="tdk">Despachante Expo</td><td class="tdv">{{ $datos['customAgent'] ?? '-' }}</td></tr>
                <tr class="row-alt">
                ...
                <tr><td class="tdk">Armador</td><td class="tdv">{{ $datos['armador'] ?? '-' }}</td></tr>
                <tr class="row-alt"><td class="tdk">Nave y Viaje</td><td class="tdv">{{ $datos['vessel'] ?? '-' }} - {{ $datos['voyage'] ?? '-' }}</td></tr>

...
class ServiceSatelital extends Controller
{
    ...
    public function flotaClienteNew($id)
{
    set_time_limit(120);

    $curl = curl_init();
    $url = 'https://app.akercontrol.com/ws/flota/2612128105/E6HW19';
    ...
    foreach ($datos as $dato) {
        ...
        if (empty($dato->patente)) continue;

        $trucks = DB::table('trucks')
            ->leftJoin('asign', 'trucks.domain', '=', 'asign.truck')
            ->leftJoin('transports', 'trucks.transport_id', '=', 'transports.id')
            ->leftJoin('drivers', 'asign.driver', '=', 'drivers.nombre')
            ->leftJoin('cntr', 'asign.cntr_number', '=', 'cntr.cntr_number')
            ->leftJoin('carga', 'cntr.booking', '=', 'carga.booking')
        ...
        $camiones[] = [
            "model"          => $trucks->model,
            "domain"         => $trucks->domain,
            "year"           => $trucks->year,
            "vto_poliza"     => $trucks->vto_poliza,
            "razon_social"   => $trucks->razon_social,
            "logo"           => $trucks->logo,
            "vto_permiso"    => $trucks->vto_permiso,
            "titulo"         => "{$trucks->domain} - {$trucks->razon_social}",

            "ult_latitud"    => $dato->ult_latitud,
            "ult_longitud"   => $dato->ult_longitud,
            "ult_velocidad"  => $dato->ult_velocidad,
            "ult_fecha"      => $dato->ult_fecha,
            "ult_reporte"    => $dato->ult_reporte,
            "ult_direccion"  => $dato->ult_direccion,
            "direccion"      => $dato->ult_direccion,

            "cntr" => [
                "contenedor"    => $trucks->contenedor,
                "type"          => $trucks->tipoContenedor,
                "main_status"   => $trucks->main_status,
                "status_detail" => $trucks->status_cntr,
            ],

            "general" => [
                "booking"       => $trucks->booking,
                "type"          => $trucks->cargaType,
                "retiro_place"  => $trucks->load_place,
                "commodity"     => $trucks->commodity,
                "ref_customer"  => $trucks->ref_customer,
                "agent_port"    => $trucks->agent_port,
                "id_carga"      => $trucks->cargaId,
                "url_carga"     => env('FRONT_URL') . '/includes/view_carga_user.php?id=' . $trucks->cargaId,
            ],

            "origen" => [
                "description" => $trucks->load_place,
                "lat"         => $trucks->LoadPlaceLat,
                "lng"         => $trucks->LoadPlaceLng,
                "load_date"   => $trucks->load_date,
            ],

            "destino" => [
                "description" => $trucks->unload_place,
                "lat"         => $trucks->UnloadPlaceLat,
                "lng"         => $trucks->UnloadPlaceLng,
                "load_date"   => $trucks->unload_date,
            ],

            "aduana" => [
                "description" => $trucks->custom_place,
                "lat"         => $trucks->aduanaLat,
                "lng"         => $trucks->aduanaLng,
                "load_date"   => $trucks->load_date,
            ],

            "driver" => [
                "nombre"    => $trucks->driver,
                "documento" => $trucks->documento,
                "carnet"    => $trucks->vto_carnet,
                "whatsapp"  => $trucks->WhatsApp,
            ],
        ];
    }

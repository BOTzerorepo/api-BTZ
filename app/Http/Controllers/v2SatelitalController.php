...
class v2SatelitalController extends Controller
{
    public function serviceSatelital()
    {
        ...
        $viajes = DB::table('trucks')
            ...
            ->get([
                ...
                'L.latitud AS carga_lat',
                'L.longitud AS carga_lng',
                DB::raw('COALESCE(a_impo.lat,a_exp.lat) AS aduana_lat'),
                DB::raw('COALESCE(a_impo.lon,a_exp.lon) AS aduana_lng'),
                'U.latitud AS descarga_lat',
                'U.longitud AS descarga_lng',
            ]);

        if ($viajes->isEmpty()) {
            Log::debug('serviceSatelital: no hay viajes activos');
            return;
        }
        ...
        foreach ($viajes as $v) {
            ...
            try {
                ...
                if (!isset($body['data'][$v->domain]['ult_latitud']))
                    continue;
        ...
        Log::debug('serviceSatelital: END');
    }
    private function checkPoint($v, $tipo, $dist, $TH_IN, $TH_OUT, $lat, $lng, $status)
    {
        ...
        if ($entering) {
            ...
            Log::debug("ENTER {$tipo} - {$v->cntr_number}");
            $this->logGeoAction([
                'trip_id' => $v->trip_id,
                'cntr_number' => $v->cntr_number,
                'domain' => $v->domain,
        ...
        if ($exiting) {
            ...
            Log::debug("EXIT {$tipo} - {$v->cntr_number}");
            $this->logGeoAction([
                'trip_id' => $v->trip_id,
                'cntr_number' => $v->cntr_number,
                'domain' => $v->domain,
    ...
    private function fireEndpoint($http, $url, $label, $tripId)
    {
        try {
            $http->post($url);
            Log::debug("📡 $label ejecutado para viaje $tripId");
        } catch (\Throwable $e) {
